...
import logging
logger = logging.getLogger('dragonfly.custom.digibox_interface')

class DigiboxCoordinator(dripline.core.Endpoint):
    '''
    Coordinated interactions with all instruments within the broader digibox system.
    Provides single point of contact to digibox
    '''
    def __init__(self, enable_output_sets=None, disable_output_sets=None, sag_injection_sets=None, switch_endpoint=None, extra_logs=[], state_extra_logs={}, **kwargs):
        ...
        self.digibox_parameter_sets = digibox_parameter_sets
# not sure if something like this is necessary yet
#        self.switch_endpoint = switch_endpoint
#        self.extra_logs = extra_logs
#        self.state_extra_logs = state_extra_logs

# don't know what this does, but looks important
    ...
    def set_single_endpoint(self,this_set,values):
        self.provider.set(this_set,this_value)
    ...
    def update_state(self, new_state):
        ...
        # for the digibox this means setting the frequency, quality factor and resolution
        if new_state == 'freq':
            self.provider.set(self.digi_endpoint,"freq")
        if new_state == 'term':
