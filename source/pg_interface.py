...
from dripline.core import Gogol, exceptions # for sensor_logger
from dripline.core.exceptions import *
from dragonfly.implementations import SensorLogger

import logging
logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)

__all__.append('PostgreSQLInterface_ADMX')
__all__.append("SQLTable_ADMX")
__all__.append('SQLSnapshot_ADMX')
__all__.append('SensorLogger_ADMX')

#@fancy_doc
class PostgreSQLInterface_ADMX(Provider):
    '''
    ...
    def add_endpoint(self, endpoint):
        ...
        if isinstance(endpoint, SQLTable_ADMX):
            ...
            print("%%%%%%%%%%%%%%%% the table table is: {}".format(endpoint.table))
        if isinstance(endpoint, SQLSnapshot_ADMX):
...
class SQLTable_ADMX(Endpoint):
    ...
    def __init__(self, table_name, schema,
        ...
        required_insert_names (list): list of names (str||dict) of the table columns which must be included on every requested insert (if dict: keys are 'column' and 'payload_key', if string it is assumed both are that value)
        return_col_names (list): list of names (str||dict) of columns whose values should be returned on completion of the insert (if dict: keys are 'column' and 'payload_key', if string it is assumed both are that value)
        optional_insert_names (list): list of names (str||dict) of columns which the user may specify on an insert request, but which may be omitted (if dict: keys are 'column' and 'payload_key', if string it is assumed both are that value)
    ...
    def do_select(self, return_cols=[], where_eq_dict={}, where_lt_dict={}, where_gt_dict={}):
        ...
        print("self.table is: {}".format(self.table))
        print("self.table.c is: {}".format(self.table.c))
    ...
    def do_insert(self, *args, **kwargs):
        ...
        for col in kwargs.keys():
            if (not col in [item['payload_key'] for item in self._required_insert_names]) and (not col in [item['payload_key'] for item in self._optional_insert_names]):
        ...
        for col in self._required_insert_names:
            if not col['payload_key'] in kwargs.keys():
                raise DriplineDatabaseError('a payload value for <{}> is required!\ngot: {}'.format(col, kwargs))
...
#@fancy_doc
class SQLSnapshot_ADMX(SQLTable_ADMX):
    '''
    '''
    ...
    def __init__(self, target_items, payload_field='value_cal', *args, **kwargs):
        ...
        SQLTable_2.__init__(self, *args, **kwargs)
...
## adding SensorLogger
class SensorLogger_ADMX(Gogol, PostgreSQLInterface_ADMX):
    '''
    ...
    def __init__(self, sensor_type_map_table, data_tables_dict={}, **kwargs):
        ...
        PostgreSQLInterface_ADMX.__init__(self, **kwargs)
    ...
    def add_endpoint(self, endpoint):
        ...
        print("mro: {}".format(inspect.getmro(type(self))))
        print("adding endpoint: {}".format(endpoint))
        # establish Spimescape add_endpoint as a starter
        print("Gogol typecheck: {}".format(isinstance(self, Gogol)))
        print("PostgreSQLInterface typecheck: {}".format(isinstance(self, PostgreSQLInterface_ADMX)))
        Gogol.add_endpoint(self, endpoint)
        # forcing PostgreSQLInterface add_endpoint usage
        print("self is: {}".format(self))
        print("PostgreSQLInterface typecheck: {}".format(isinstance(self, PostgreSQLInterface_ADMX)))
        print("endpoint is: {}".format(endpoint))
        PostgreSQLInterface_ADMX.add_endpoint(self, endpoint)
    ...
    def this_consume(self, message, basic_deliver):
        ...
        this_type = None
        print('the tables are >>>>>>>>>> {}'.format(self.endpoints))
        print("numeric table is >>>>>>>> {}".format(self.endpoints['numeric_table']))
        print("id_map table is >>>>>>>>> {}".format(self.endpoints['id_map']))
        print("\n\n trying to get sensor type from table: {}".format(self.endpoints[self._sensor_type_map_table]))
        this_table = self.endpoints[self._sensor_type_map_table]
        print('\n\n that is: {}'.format(this_table))
        ###import pdb;pdb.set_trace()
        this_type = this_table.do_select(return_cols=['epics_data_type'],
                                         where_eq_dict={'sensor_name':sensor_name},
                                        )
        if not this_type[1]:
